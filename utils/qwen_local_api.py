...
class QwenVLClient:
    ...
    def __init__(
        ...
        if use_flash_attention:
            try:
                ...
                logger.info("Flash attention not available, falling back to default: %s", e)
        ...
        logger.info("Loaded Qwen VL local model: %s", model_id)
    ...
    def get_instance(
        ...
        if os.path.isdir(os.path.join(model_id, "huggingface")):
            ...
            logger.info("Detected 'huggingface' subdirectory, updated model_id to: %s", model_id)
...
def get_qwen_chat_completion(
    ...
    **kwargs,
    ...
    if "checkpoint-450-merged" in model or "grpo" in model.lower():
        ...
        logger.info("[QwenLocal] Detected checkpoint-450-merged/grpo model, applying penalties: "
                    "presence=%s, frequency=%s", gen_kwargs['presence_penalty'], gen_kwargs['frequency_penalty'])
    ...
    if do_sample:
        ...
        logger.info("[QwenLocal] Using greedy decoding (temperature=0, do_sample=False)")
