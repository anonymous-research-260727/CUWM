...
from utils.tool_definitions import normalize_tool_args, normalize_tool_args_a11y

logger = logging.getLogger(__name__)

COORDINATE_TOLERANCE = 25.0
DEFAULT_RESOLUTION = (1024, 736)
...
class ActionEvaluation:
    ...
    def _preprocess_command(self, action: dict, resolution=DEFAULT_RESOLUTION) -> dict:
        logger.debug("Original action: %s", action)
        if action["function"] == "drag":
            ...
            action["args"]["rectangle"] = {
                "left": max(0, start_x) - COORDINATE_TOLERANCE,
                "top": max(0, start_y) - COORDINATE_TOLERANCE,
                "right": min(start_x + COORDINATE_TOLERANCE, resolution[0]),
                "bottom": min(start_y + COORDINATE_TOLERANCE, resolution[1]),
            }

            action["args"]["rectangle_end"] = {
                "left": max(0, end_x) - COORDINATE_TOLERANCE,
                "top": max(0, end_y) - COORDINATE_TOLERANCE,
                "right": min(end_x + COORDINATE_TOLERANCE, resolution[0]),
                "bottom": min(end_y + COORDINATE_TOLERANCE, resolution[1]),
            }
        else:
    ...
    def _compare_drag_args(
        ...
        try:
            ...
            logger.error("Error comparing drag arguments: %s", e)
            return False
    ...
    def _compare_regular_args(
        ...
        try:
            ...
            logger.error("Error comparing regular arguments: %s", e)
            return False
...
class ActionEvaluationA11y:
    ...
    def _compare_a11y_args(
        ...
        try:
            ...
            logger.error("Error comparing A11Y arguments: %s", e)
            return False
