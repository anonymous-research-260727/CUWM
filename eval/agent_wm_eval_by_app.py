...
def get_gt_action(folder, eval_dir, source_data_root):
...
def evaluate_actions(filenames, folders, eval_dir, source_data_root, evaluator):
    overall_result = {}
    skipped_folders = set()
    ...
    for filename in filenames:
        ...
        for folder in tqdm(folders):
            ...
            gt_command, gt_raw = get_gt_action(folder, eval_dir, source_data_root)
    ...
    # summary the results by App
    # App is determined by the first directory under eval_dir
    summary_result_by_app = {}
    
    for filename, folder_results in overall_result.items():
        summary_result_by_app[filename] = {}
        
        # Temporary storage to aggregate sums and counts per app
        app_stats = {} 
        ...
        for folder, metrics in folder_results.items():
            ...
            if app_name not in app_stats:
                app_stats[app_name] = {"count": 0, "sums": {}}
            
            app_stats[app_name]["count"] += 1
            
            for key, value in metrics.items():
                if key != "pred_action_idx":
                    if key not in app_stats[app_name]["sums"]:
                        app_stats[app_name]["sums"][key] = 0.0
                    app_stats[app_name]["sums"][key] += value

        # specific summary for "overall" (all apps combined)
        # Note: The original script produced a flat summary for the filename. 
        # Here we want to keep it separated by app.
        
        for app_name, stats in app_stats.items():
            summary_result_by_app[filename][app_name] = {}
            count = stats["count"]
            for key, value in stats["sums"].items():
                summary_result_by_app[filename][app_name][key] = value / count
            summary_result_by_app[filename][app_name]["count"] = count

    # Also Generate the original summary for backward compatibility/reference
    summary_result = {}
    for filename, folder_results in overall_result.items():
    ...
    with open(os.path.join(eval_dir, "eval_result/agent_wm_evaluation_summary_by_app.json"), "w") as f:
        json.dump(summary_result_by_app, f, indent=2)

    with open(os.path.join(eval_dir, "eval_result/agent_wm_evaluation_summary.json"), "w") as f:
        json.dump(summary_result, f, indent=2)
    
    return overall_result, summary_result_by_app

def get_best_worse_score(filename, folders, eval_dir, source_data_root, evaluator):
    overall_result = {}
    skipped_folders = set()
...
if __name__ == "__main__":
    ...
    source_data_root = os.environ.get("SOURCE_DATA_ROOT", "data/new_prompt/test")
    ...
    print(len(folders), "folders found for evaluation.")
    # assert 0
    
    evaluator = ActionEvaluationA11y()
    evaluate_actions(filenames, folders, eval_dir, source_data_root, evaluator)
    get_best_worse_score([filenames[0]], folders, eval_dir, source_data_root, evaluator)
