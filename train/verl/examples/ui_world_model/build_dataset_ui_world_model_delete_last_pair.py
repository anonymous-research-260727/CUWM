...
_PAIR_RE = re.compile(r"pair[_-]?(\d+)$", re.IGNORECASE)
...
def _parse_pair_num_from_dir(pair_dir: str) -> int | None:
    """
    pair_dir ends with something like .../pair_05 or .../pair-12
    Return int(05)=5, int(12)=12, else None.
    """
...
def _group_root_of_pair_dir(pair_dir: str) -> str:
    """
    /.../paired/ppt_4s_13233/pair_05  ->  /.../paired/ppt_4s_13233
    """
    return str(Path(pair_dir).parent)
...
def main():
    ...
    # ---------------------------------------------------------
    # 1) 预扫：统计每个 .../paired/<xxx>/ 下最大的 pair_x 编号（最后一个 pair）
    #    之后在构建 rows 时跳过该 pair_x
    # ---------------------------------------------------------
    max_pair_by_group: dict[str, int] = {}
    for item in data:
        imgs = item.get("images", [])
        if not imgs:
            continue
        prev_path = imgs[0]
        pair_dir = os.path.dirname(prev_path)               # .../pair_05
        group_root = _group_root_of_pair_dir(pair_dir)      # .../ppt_4s_13233
        pair_num = _parse_pair_num_from_dir(pair_dir)
        if pair_num is None:
            continue
        old = max_pair_by_group.get(group_root)
        if old is None or pair_num > old:
            max_pair_by_group[group_root] = pair_num

    rows = []
    skipped = {"no_prev": 0, "no_action": 0, "no_assistant": 0, "no_next": 0, "bad_image": 0, "drop_last_pair": 0}
    ...
    for idx, item in enumerate(data):
        ...
        pair_path = os.path.dirname(prev_path)              # .../pair_05
        group_root = _group_root_of_pair_dir(pair_path)     # .../ppt_4s_13233
        pair_num = _parse_pair_num_from_dir(pair_path)

        # ---------------------------------------------------------
        # 2) 丢弃每个 group_root 下 “最后一个 pair_x”
        # ---------------------------------------------------------
        if pair_num is not None:
            max_pair = max_pair_by_group.get(group_root)
            if max_pair is not None and pair_num == max_pair:
                skipped["drop_last_pair"] += 1
                continue
        ...
        rows.append({
            ...
            "extra_info": {
                ...
                "pair_path": pair_path,
                "a11y_path": os.path.join(pair_path, "a11y.json"),
                "prev_path": prev_path,
                "next_path": next_path,
            }
        })
    ...
    out_path = out_dir / "no_last" / f"{split}.parquet"
