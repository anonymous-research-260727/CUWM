...
**Constraint:** You MUST select a valid action index. Do not return -1 or None.
...
def _build_gui_action_scoring_prompt(
    include_text: bool,
    include_image: bool,
) -> str:
    fields = [
        "- action_idx: the unique identifier/index for this action option\n",
        "- action: the structured action command to be executed\n",
    ]

    reasoning_note = ""

    # Field definitions for World Model predictions
    if include_text:
        fields.append(
            "- predicted_state_text: a text description of the UI state after executing the action\n"
        )

    if include_image:
        fields.append(
            "- predicted_state_image: a visual prediction (mockup) showing the expected UI state after executing the action\n"
        )

    if include_text or include_image:
        reasoning_note = (
            "The predicted_state_text and/or predicted_state_image are generated by a world model. "
            "They may not be faithful representations of the real UI and may contain visual noise (e.g., garbled text, incomplete dialogs, or slight distortions) due to the diffusion generation process. "
            "**Do NOT penalize these minor visual artifacts** if the overall state transition is correct (e.g., the correct menu is open, the correct cell is highlighted, or the layout has shifted appropriately). "
            "Your evaluation should focus on whether the *semantics* of the UI state have changed in the right direction, rather than pixel-perfect clarity. "
            "Only lower your confidence if the prediction is completely incoherent or shows a hallucinated state that contradicts the goal.\n\n"
        )

    return f"""
You are given a list of candidate actions to evaluate. Each action contains the following details:
{"".join(fields)}


**STEP 1: PER-OPTION SCORING**
For each option, calculate a **Confidence Score (0.0 to 1.0)** based on two factors:

1.  **Semantic Relevance (50% Weight): Is the command logical?**
    * Does the `action` command targeting the correct element? (e.g., Does it target the specific cell mentioned in the goal?)
    * Does the command type match the goal? (e.g., `set_cell_value` or `type` for editing, rather than just `click`).

2.  **Visual Reliability (50% Weight): Does the image show a state transition in the right direction towards the goal**
    * **Evaluate the Visual Delta:** Compare the current screenshot to the `predicted_state_image`. Does the change represent a meaningful step forward in the workflow?
    * **Right Direction:** 
      * If the goal is to "Edit cell D4", a valid transition is the cell becoming highlighted, a cursor appearing, or the value changing.
      * If the goal is "Insert Chart", a valid transition is the "Insert" tab being selected or a dropdown menu opening.
    * **Scoring:** Give a high score if the image shows the UI state *moving closer* to the goal (progression), even if the final result isn't fully visible yet. Lower the score if the state appears static (no change) or moves to an unrelated screen.

{reasoning_note.rstrip()}

The actions are provided below in the format:
Action Option <N>:
 - Action: ...
 - Predicted State ...

Return a single JSON **list** of objects, where each object corresponds to one candidate action:
[
  {{
    "action_idx": <int, matching the input index N>,
    "judgment": "valid" or "invalid",
    "confidence": <float 0.0-1.0>,
    "reasoning": "<Concise explanation of the score, referencing specific semantic or visual details>"
  }},
  ...
]
...
Here are the candidate actions to evaluate:
""".strip()

GUI_ACTION_SCORING_NONE_PROMPT = _build_gui_action_scoring_prompt(
    include_text=False,
    include_image=False,
)

GUI_ACTION_SCORING_TEXT_PROMPT = _build_gui_action_scoring_prompt(
    include_text=True,
    include_image=False,
)

GUI_ACTION_SCORING_IMAGE_PROMPT = _build_gui_action_scoring_prompt(
    include_text=False,
    include_image=True,
)

GUI_ACTION_SCORING_TEXT_IMAGE_PROMPT = _build_gui_action_scoring_prompt(
    include_text=True,
    include_image=True,
)

GUI_ACTION_SCORING_PROMPT = {
    "none": GUI_ACTION_SCORING_NONE_PROMPT,
    "text": GUI_ACTION_SCORING_TEXT_PROMPT,
    "image": GUI_ACTION_SCORING_IMAGE_PROMPT,
    "text+image": GUI_ACTION_SCORING_TEXT_IMAGE_PROMPT
}



GUI_TOP_2_SELECTION_PROMPT_TEMPLATE = """
You are an AI assistant tasked with selecting the optimal next action on an Android device from two high-scoring candidates.
...
def build_top_2_selection_prompt(
    include_text_prediction: bool,
    include_image_prediction: bool,
) -> str:
    fields = [
        "- action: the structured action command\n",
        "- rationale: the reasoning for why this action was originally proposed\n"
    ]

    reasoning_note = ""

    # Field definitions for World Model predictions
    if include_text_prediction:
        fields.append(
            "- predicted_state_text: a brief textual prediction of the expected UI state after executing the action\n"
        )
    
    if include_image_prediction:
        fields.append(
            "- predicted_state_image: a visual prediction (mockup) showing the expected UI state after executing the action\n"
        )

    if include_text_prediction or include_image_prediction:
        reasoning_note = (
            "The predicted_state_text and/or predicted_state_image fields are predictions of the UI state after executing the action. "
            "They may not be fully accurate, but you may use them as references when making your decision. "
            "If you believe they are unreliable or inconsistent with the screenshot or task, you may ignore them.\n\n"
        )

    return f"""
{GUI_TOP_2_SELECTION_PROMPT_TEMPLATE.strip()}

You are given a list of two candidate actions, each with:
{"".join(fields)}
...
Please return your final answer as a JSON object with the following format:
{{
  "action_idx": <0 or 1, representing the index of the selected action>,
  "thought": "<Detailed explanation of your choice, considering the hints and history>"
}}

Do NOT include anything else outside the JSON object.

Here are the two candidate actions:
""".strip()

GUI_TOP_2_SELECTION_NONE_PROMPT = build_top_2_selection_prompt(
    include_text_prediction=False,
    include_image_prediction=False,
)

GUI_TOP_2_SELECTION_TEXT_PROMPT = build_top_2_selection_prompt(
    include_text_prediction=True,
    include_image_prediction=False,
)

GUI_TOP_2_SELECTION_IMAGE_PROMPT = build_top_2_selection_prompt(
    include_text_prediction=False,
    include_image_prediction=True,
)

GUI_TOP_2_SELECTION_TEXT_IMAGE_PROMPT = build_top_2_selection_prompt(
    include_text_prediction=True,
    include_image_prediction=True,
)

GUI_TOP_2_SELECTION_PROMPT = {
    "none": GUI_TOP_2_SELECTION_NONE_PROMPT,
    "text": GUI_TOP_2_SELECTION_TEXT_PROMPT,
    "image": GUI_TOP_2_SELECTION_IMAGE_PROMPT,
    "text+image": GUI_TOP_2_SELECTION_TEXT_IMAGE_PROMPT
}
